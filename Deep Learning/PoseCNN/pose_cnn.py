...
class SegmentationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 64):
        ...
        self.conv3 = nn.Conv2d(hidden_layer_dim, num_classes + 1, 1)
        ...
        kaiming_normal_(self.conv3.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, feature1, feature2):
        ...
        x2 = self.relu1(self.conv2(feature2))
...
class TranslationBranch(nn.Module):
    ...
    def __init__(self, num_classes = 10, hidden_layer_dim = 128):
        ...
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(512, hidden_layer_dim, 1)
        self.conv3 = nn.Conv2d(hidden_layer_dim, 3*num_classes, 1)
        self.softmax = nn.Softmax(dim=1)
        kaiming_normal_(self.conv1.weight, mode='fan_out', nonlinearity='relu')
        nn.init.zeros_(self.conv1.bias)
        kaiming_normal_(self.conv2.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, feature1, feature2):
        ...
        # translation = nn.functional.interpolate(translation,size=(480,640), mode='bilinear',align_corners=True)
        # segmentation = torch.argmax(probability, dim=1)
        # bbx = self.label2bbx(segmentation) 
        ...
        x1_upsampled = nn.functional.interpolate(x1, size=(x2.shape[2], x2.shape[3]), mode='bilinear', align_corners=True)
        x = x1_upsampled + x2    
        translation = nn.functional.interpolate(x,size=(480,640), mode='bilinear',align_corners=True)
        translation = self.conv3(translation) 
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
        return translation
...
class RotationBranch(nn.Module):
    ...
    def __init__(self, feature_dim = 512, roi_shape = 7, hidden_dim = 4096, num_classes = 10):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.roi_pool1 = RoIPool((roi_shape,roi_shape),spatial_scale=1/8)
        self.roi_pool2 = RoIPool((roi_shape,roi_shape),spatial_scale=1/16)
    ...
    def forward(self, feature1, feature2, bbx):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        x1 = self.roi_pool1(feature1.float(),bbx.float())
        x2 = self.roi_pool2(feature2.float(),bbx.float())
        x = x1 + x2
        x_flat = x1.view(x.size(0), -1)
...
class PoseCNN(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            # Replace "pass" statement with your code
            f1 , f2 = self.feature_extraction(input_dict)
            prob , segmentation , bbx = self.segmentation_branch(f1 , f2)
            loss_dict["loss_segmentation"] = loss_cross_entropy(prob,input_dict["label"])
            translation_map = self.translation_branch(f1,f2)
            ...
            if len(selected_bbx) != 0:
              quaternion = self.rotation_branch(f1,f2,selected_bbx[:,:-1])
            ...
            with torch.no_grad():
              ...
              translation_map = self.translation_branch(f1,f2)
              centers , depths = HoughVoting(segmentation, translation_map)
              bbx = bbx.to(torch.cuda.LongTensor())
              quaternion = self.rotation_branch(f1, f2, bbx[:,:-1])
